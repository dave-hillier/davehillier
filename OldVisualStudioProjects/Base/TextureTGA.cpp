...
void TextureTGA::ReadRGB(std::ifstream &i)
{
	...
	pixelData.resize( header.width * header.height * bytesPerPixel );

	i.read(&(*pixelData.begin()), (std::streamsize)pixelData.size());
...
void TextureTGA::ReadRLE(std::ifstream &i)
{
...
std::ifstream& operator >>(std::ifstream &i, TextureTGA &t)
{
	...
	if (t.header.imageType == TextureTGA::Header::RGB)
		t.ReadRGB(i);
	else if (t.header.imageType == TextureTGA::Header::RGBRLE)
		t.ReadRLE(i);;
	return i;
}

...
vector.
*/
void Quat::ToVector(Vector &dest)
{
   Normalize();
...
*/
void Quat::AxisAngleToQuat(const Vector &axis, float angle)
{
...
void Quat::EulerToQuat(float x, float y, float z)
{
   ...
   qx.x = (float)sin(x / 2.0f);
   qx.w = (float)cos(x / 2.0f);

   qy.y = (float)sin(y / 2.0f);
   qy.w = (float)cos(y / 2.0f);

   qz.z = (float)sin(z / 2.0f);
   qz.w = (float)cos(z / 2.0f);

...
float bgColor[4] = {1.0, 0.5, 0.5, 1.0};
float gdColor[4] = {0.3, 0.3, 0.3, 1.0};
int gradient = 0, italic = 0, bold = 0, shadow = 0, textsize=12;
GLfloat tIncX;
GLfloat tIncY;
...
int SetEffectColorFromToken (char *s)
{
   int clr[4];
   int ret = 1;
   ...
   if (*s == 'G')
   {
      ...
      if (sscanf(s, "(%d %d %d)", &clr[0], &clr[1], &clr[2]) != 3)
         return -1;
      gdColor[0] = clr[0] * fontITOF;
	  gdColor[1] = clr[1] * fontITOF;
	  gdColor[2] = clr[2] * fontITOF;
   }
   if (*s == 'D')
   {
      s += 1;
      if (sscanf(s, "(%d %d %d)", &clr[0], &clr[1], &clr[2]) != 3)
         return -1;
      bgColor[0] = clr[0] * fontITOF;
	  bgColor[1] = clr[1] * fontITOF;
	  bgColor[2] = clr[2] * fontITOF;
   }
...
int SlashParser (char *buffPtr, int *x, int *y)
{
   ...
   *buffPtr++;
   ...
   {
      case 's':
		  return SetSizeFromToken(buffPtr);
      break;
      case 'a':
      case 'c':
         *x -= textsize;
      ...
      case 'D':
         *x -= textsize;
         return SetEffectColorFromToken(buffPtr);
      break;
	  case 'i':
         *x -= textsize;
      ...
      case 'd':
         *x -= textsize;
         return ShadowMode(buffPtr);
      break;
      case 'g':
         *x -= textsize;
         return GradientMode(buffPtr);
      break;
      case 'b':
         *x -= textsize;
      ...
      default:
         *buffPtr--;
         RenderChar(*buffPtr, *x, *y, textsize);
         return ret;
      break;
   }
}
void WalkString (int xpos, int ypos, char *buffPtr)
{
	...
	for ( ; *buffPtr; *buffPtr++, x += textsize)
	{	
		switch (*buffPtr)
		{
		case '\n':

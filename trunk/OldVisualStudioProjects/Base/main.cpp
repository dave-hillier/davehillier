...
namespace Base {
	...
	public:
		...
		{
			//test.AddNode(Vector3( 0, 40, 0 ));
			//test.AddNode(Vector3( 40, 40, 0 ));
			//test.AddNode(Vector3( 40, 80, 0 ));
			//test.AddNode(Vector3( -30, 60, 0 ));
			//test.AddNode(Vector3( -30, 30, 0 ));
			//test.AddNode(Vector3( -30, 90, 0 ));
			//test.AddNode(Vector3( -60, 70, 0 ));
			...
			test.AddNode(Vector3(-25, 25, 0));
			test.AddNode(Vector3(25, 25, 0));
			test.AddNode(Vector3(0, 50, 0));
		...
		void Render()
		{
			...
			glTranslatef(0, 0, 0);
//			const char *dave = "Str TEST here";
//			glCallLists(strlen(dave), GL_BYTE, dave); 
			glPopMatrix();
...
//	Base::debugLog << "String " << (int)10190101 << " " << (float)3.1400000000000009f << std::endl;
	...
	std::ifstream file("Data/test_24bit.tga", std::ios::binary|std::ios::in);
	if (file.is_open() && !file.eof())
	{
		file >> tga;
		file.close();
	}
	tex = Texture(tga.data(), tga.bytesPerPixel(), tga.width(), tga.height());
